...
class SyncClass 
{
    ...
    # method to find value of a control property - pass window name, control name and property name
    [string]GetControlPropertyValue($windowName, $controlName, $propertyName){
        $control = $this.SyncHash.$windowName.FindName($controlName)
        if($control -ne $null){
...
foreach($x in $vx)
{
    ...
    $xaml.SelectNodes("//*[@Name]") | %{ #find all nodes with a "Name" attribute
        ...
        $Global:controls += (Get-Variable -Name "form$($x)Control$($_.Name)").Name #add the control name to our array
...
Function NewStackPanelControl(){
    ...
    $Global:controls += (Get-Variable -Name "formMainWindowControl$($name.Substring(1))").Name #add the control name to our array
    $SyncClass.SyncHash.Add($cname, $TextBox) #add the control directly to the hashtable
    #>
    #return $Global:TextBox
...
$formMainWindowControlStartButton.add_Click({
    ...
    # try to get value value from dynamic control 
